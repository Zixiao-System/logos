...
- **VS Code 兼容扩展**: 复用生态，兼容现有 `.vsix` 与 API 子集。
- **WASM 扩展**: 受控运行时（权限更细、能力更窄），用于性能敏感或安全敏感场景。
- 两类扩展可并行启用，长期目标为逐步将关键能力迁移到 WASM 扩展标准。
...
- 与主进程通过 IPC 通信
- 崩溃后可自动重启
- 兼容宿主切换：可通过环境变量 `LOGOS_EXT_HOST_MODE=vscode` 试运行 VS Code Extension Host
...
- 新安装扩展默认处于「未授权」状态，不会被扩展宿主加载
- 用户首次启用时弹出权限确认，明确提示扩展可执行本地代码并访问文件/网络/终端
- 用户拒绝授权时，扩展保持禁用状态并可随时重新授权
